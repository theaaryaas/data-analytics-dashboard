...
@Component({
  ...
  template: `
    <div style="min-height:100vh;background:linear-gradient(135deg,#FFF8F5,#FFF5F0,#FFF0E6);display:flex;align-items:center;justify-content:center;padding:20px;position:relative;overflow:hidden">
      ...
      <mat-card style="width:100%;max-width:450px;border-radius:20px;border:1px solid rgba(255,179,167,.3);box-shadow:0 12px 40px rgba(255,112,67,.2);background:linear-gradient(135deg,rgba(255,255,255,.98),rgba(255,248,245,.98));backdrop-filter:blur(20px);position:relative;z-index:1;padding:40px">
        <div style="text-align:center;margin-bottom:40px">
          ...
          <h2 style="color:#4A5568;margin:0 0 8px;font-size:32px;font-weight:700;letter-spacing:-.5px">Welcome Back</h2>
          <p style="color:#7F8C8D;font-size:15px;margin:0">Sign in to your Data Analytics account</p>
        </div>

        <form [formGroup]="loginForm" (ngSubmit)="onLogin()" style="display:flex;flex-direction:column;gap:20px">
          ...
          <button mat-raised-button type="submit" [disabled]="loading||loginForm.invalid"
        ...
        <div style="text-align:center;margin-top:32px;padding-top:24px;border-top:1px solid rgba(255,179,167,.2)">
          <p style="color:#7F8C8D;margin:0;font-size:14px">
            ...
            <a routerLink="/signup" style="color:#FF8A65;font-weight:600;text-decoration:none;margin-left:4px">Sign Up</a>
          </p>
        </div>
      </mat-card>
    </div>`,
...
export class LoginComponent {
  ...
  f(name: string) { return this.loginForm.get(name); }
  ...
  onLogin() {
    ...
    setTimeout(() => {
      ...
      this.authService.login({ email, name: email.split('@')[0] });
