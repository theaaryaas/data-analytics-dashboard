  ...
  template: `
    <div>
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px">
        <h2 style="margin:0">MongoDB Docker</h2>
      ...
      <app-file-upload [targetDatabase]="'mongodb'" (onUploadSuccess)="fetchFiles()"></app-file-upload>
      ...
      <mat-card style="margin-top:24px;border-radius:12px;border:1px solid rgba(255,179,167,.3);background:linear-gradient(135deg,rgba(255,255,255,.7),rgba(255,248,245,.7));backdrop-filter:blur(10px)">
        <mat-card-header>
          <mat-card-title style="color:#4A5568;font-weight:600">Files in MongoDB Docker ({{ files.length }})</mat-card-title>
        </mat-card-header>
        <mat-card-content>
...
export class MongoDBPageComponent implements OnInit {
  files: FileInfo[] = [];
  ...
  fetchFiles() {
    this.apiService.getMongoDBFiles().subscribe({
      next: r => {
        this.files = (r?.files || []).map((f: any) => ({ id: f._id || f.id, filename: f.filename, file_type: f.file_type, columns: f.columns || [], row_count: f.row_count || 0, uploaded_at: f.uploaded_at, preview: f.preview_data || f.preview || [], stats: f.stats, source: 'mongodb' }));
        if (this.files.length <= this.pageIndex * this.pageSize) this.pageIndex = 0;
      },
      error: e => { console.error('Failed to fetch MongoDB files:', e); this.files = []; this.pageIndex = 0; }
    });
  }
